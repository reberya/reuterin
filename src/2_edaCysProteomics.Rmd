...
# select needed cols
cyst <- p2[,c(2,6,23,31, 10:21)]
colnames(cyst)[1:3] <- c('gene', 'gene_site', 'avg_cntrl', 'cr_Reut_100')

# Change zero values to matrix min
sub <- as.matrix(cyst[,5:16])
vals <- unlist(sub)
vals <- vals[vals > 0]
...
## compute group SDs
cyst %<>% 
  mutate(DMF_25uM_sd = rowSds(as.matrix(cyst[,5:7]))) %>% 
  mutate(DMF_100uM_sd = rowSds(as.matrix(cyst[,8:10]))) %>% 
  mutate(Reut_50uM_sd = rowSds(as.matrix(cyst[,11:13]))) %>% 
  mutate(Reut_100uM_sd = rowSds(as.matrix(cyst[,14:16])))
## select max sd and arrange by max sd
cyst %<>%
  mutate(maxsd = rowMaxs(as.matrix(cyst[,17:20]))) %>%
  arrange(maxsd) %>%
...
cyst[,5:16] <- cyst$avg_cntrl/cyst[,5:16]

# Select top 100 gene_sites
cyst %<>% arrange(-`CR 100 Reut`)
values <- cyst[1:100, 5:16]
...
col_fun = colorRamp2(c( 0, 2.5, 5), c("white", 'orange', "red"))
